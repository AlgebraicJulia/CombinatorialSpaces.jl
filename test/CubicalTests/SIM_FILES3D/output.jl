...
# ── DataHandler ───────────────────────────────────────────────────────────────
const io_stream = DataStream("sim3D", OUTFILE, Datum[
...
#     # Mid-plane y-index for 2D slice visualisations
#     y_mid = div(NYB, 2) + 1
...
#             ic    = dset[1,      :, y_mid, :]
#             final = dset[ntimes, :, y_mid, :]

#             for (data, label) in ((ic, "IC"), (final, "final"))
#                 fig = Figure(; size = (700, 600))
#                 ax  = Axis(fig[1, 1]; title = "$field y-slice | $label", xlabel = "x", ylabel = "y")
#                 hm  = heatmap!(ax, data)
#                 Colorbar(fig[1, 2], hm)
#                 save(joinpath(IMGDIR, "$(field)_$(lowercase(label)).png"), fig)
#             end

#             let frame_obs = Observable(dset[1, :, y_mid, :])
#                 fig = Figure(; size = (700, 600))
#                 ax  = Axis(fig[1, 1]; xlabel = "x", ylabel = "y")
#                 hm  = heatmap!(ax, frame_obs)
#                 Colorbar(fig[1, 2], hm)
#                 record(fig, joinpath(IMGDIR, "$(field).mp4"), 1:ntimes; framerate = 15) do i
#                     frame_obs[] = dset[i, :, y_mid, :]
#                     ax.title[]  = "$field y-slice | t = $(round((i-1)*SAVETIME, digits=3))"
#                 end
#             end
#         end
...
#                 ("momentum_X", ic_X[:, y_mid, :], final_X[:, y_mid, :]),
#                 ("momentum_Y", ic_Y[:, y_mid, :], final_Y[:, y_mid, :]),
#                 ("momentum_Z", ic_Z[:, y_mid, :], final_Z[:, y_mid, :]))

#             cr_min, cr_max = minimum(final_data), maximum(final_data)
#             cr = cr_min == cr_max ? (cr_min - FT(1), cr_max + FT(1)) : (cr_min, cr_max)

#             for (data, label) in ((ic_data, "IC"), (final_data, "final"))
#                 fig = Figure(; size = (700, 600))
#                 ax  = CairoMakie.Axis(fig[1, 1]; title = "$comp_label y-slice | $label", xlabel = "x", ylabel = "y")
...
#                                                         comp_label == "momentum_Y" ? 2 : 3][:, y_mid, :])
#                 fig = Figure(; size = (700, 600))
...
#                     frame_obs[] = momentum_xyz(i)[comp_idx][:, y_mid, :]
#                     ax.title[]  = "$comp_label y-slice | t = $(round((i-1)*SAVETIME, digits=3))"
#                 end
#             end
#         end
