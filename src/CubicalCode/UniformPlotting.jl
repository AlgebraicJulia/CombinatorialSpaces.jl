...
function plot_xy_oneform(
    ...
    fig = Figure(; figure_kwargs...)
    axis_x_defaults = (title = "X Component", xlabel = "x", ylabel = "y")
    axis_y_defaults = (title = "Y Component", xlabel = "x", ylabel = "y")
...
function create_gif(
    ...
    CairoMakie.record(fig, file_name, 1:frames; framerate = framerate, record_kwargs...) do t
        return msh.color = interior(Val(0), solution(t * t_e / frames), s)
    end
end
...
function plot_dual_zeroform_slice(
    ...
    if align == Z_ALIGN
        ...
        y_pts = [real_dual_point(s, 1, ry, 1)[2] for ry in 1:nybr(s)]
        xlabel = "x"
        ylabel = "y"
    ...
    elseif align == Y_ALIGN
        data = [f[real_coord_to_boid(s, rx, slice_idx, rz)] for rx in 1:nxbr(s), rz in 1:nzbr(s)]
        x_pts = [real_dual_point(s, rx, 1, 1)[1] for rx in 1:nxbr(s)]
        y_pts = [real_dual_point(s, 1, 1, rz)[3] for rz in 1:nzbr(s)]
        xlabel = "x"
        ylabel = "z"
        ...
        data = [f[real_coord_to_boid(s, slice_idx, ry, rz)] for ry in 1:nybr(s), rz in 1:nzbr(s)]
        x_pts = [real_dual_point(s, 1, ry, 1)[2] for ry in 1:nybr(s)]
        y_pts = [real_dual_point(s, 1, 1, rz)[3] for rz in 1:nzbr(s)]
        xlabel = "y"
        ylabel = "z"
    ...
    default_heatmap_kwargs = (colormap = :jet,)
    merged_heatmap_kwargs = merge(default_heatmap_kwargs, heatmap_kwargs)

    fig = Figure(; merged_fig_kwargs...)
    ax = Axis(fig[1, 1]; aspect = DataAspect(), title = title, xlabel = xlabel, ylabel = ylabel)
