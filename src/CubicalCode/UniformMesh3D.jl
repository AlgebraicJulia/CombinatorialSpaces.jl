...
struct UniformCubicalComplex3D{FT <: AbstractFloat} <: AbstractEmbeddedCubicalComplex3D{FT}
  ...
  halo_down::Int;  halo_up::Int

  base_x::FT; base_y::FT; base_z::FT
end
...
function _validate_mesh_counts_3d(
  nxr::Int,
  nyr::Int,
  nzr::Int,
  halo_west::Int,
  ...
  nxr >= 2 || throw(ArgumentError("nxr must be at least 2; received $nxr"))
  nyr >= 2 || throw(ArgumentError("nyr must be at least 2; received $nyr"))
  nzr >= 2 || throw(ArgumentError("nzr must be at least 2; received $nzr"))

  halo_west >= 0 || throw(ArgumentError("halo_west must be nonnegative; received $halo_west"))
  halo_east >= 0 || throw(ArgumentError("halo_east must be nonnegative; received $halo_east"))
  halo_south >= 0 || throw(ArgumentError("halo_south must be nonnegative; received $halo_south"))
  halo_north >= 0 || throw(ArgumentError("halo_north must be nonnegative; received $halo_north"))
  halo_down >= 0 || throw(ArgumentError("halo_down must be nonnegative; received $halo_down"))
  halo_up >= 0 || throw(ArgumentError("halo_up must be nonnegative; received $halo_up"))

  return nothing
end

function _validate_mesh_geometry_3d(
  lx::FT,
  ly::FT,
  lz::FT,
  base_x::FT,
  ...
  isfinite(lx) && lx > zero(FT) || throw(ArgumentError("lx must be finite and positive; received $lx"))
  isfinite(ly) && ly > zero(FT) || throw(ArgumentError("ly must be finite and positive; received $ly"))
  isfinite(lz) && lz > zero(FT) || throw(ArgumentError("lz must be finite and positive; received $lz"))

  isfinite(base_x) || throw(ArgumentError("base_x must be finite; received $base_x"))
  isfinite(base_y) || throw(ArgumentError("base_y must be finite; received $base_y"))
  isfinite(base_z) || throw(ArgumentError("base_z must be finite; received $base_z"))

  return nothing
end

function UniformCubicalComplex3D(
  nxr::Int,
  nyr::Int,
  nzr::Int,
  lx::Real,
  ...
  halo_y::Int = 0,
  halo_z::Int = 0,
  halo_west::Int = halo_x,
  halo_east::Int = halo_x,
  halo_south::Int = halo_y,
  halo_north::Int = halo_y,
  halo_down::Int = halo_z,
  halo_up::Int = halo_z,
  base_x::Real = 0.0,
  base_y::Real = 0.0,
  base_z::Real = 0.0,
)
  FT = float(promote_type(
    typeof(lx),
    typeof(ly),
    typeof(lz),
  ))
  ...
  ly_ft = convert(FT, ly)
  lz_ft = convert(FT, lz)
  base_x_ft = convert(FT, base_x)
  base_y_ft = convert(FT, base_y)
  base_z_ft = convert(FT, base_z)

  _validate_mesh_counts_3d(
    nxr,
    nyr,
    nzr,
    halo_west,
  ...
  dx_ft = spacing(lx_ft, nxr)
  dy_ft = spacing(ly_ft, nyr)
  dz_ft = spacing(lz_ft, nzr)

  isfinite(dx_ft) && dx_ft > zero(FT) || throw(ArgumentError(
    "lx and nxr produce invalid x spacing: $dx_ft",
  ))
  isfinite(dy_ft) && dy_ft > zero(FT) || throw(ArgumentError(
    "ly and nyr produce invalid y spacing: $dy_ft",
  ))
  isfinite(dz_ft) && dz_ft > zero(FT) || throw(ArgumentError(
    "lz and nzr produce invalid z spacing: $dz_ft",
  ))

  return UniformCubicalComplex3D{FT}(
    nxr,
    nyr,
    nzr,
    dx_ft,
...
PseudoCubicalMesh(
  ...
  nz::Int;
  kwargs...,
) = PseudoCubicalMesh3D(nx, ny, nz; kwargs...)
...
halo_west(s::AbstractCubicalComplex3D)  = s.halo_west
halo_east(s::AbstractCubicalComplex3D)  = s.halo_east
halo_south(s::AbstractCubicalComplex3D) = s.halo_south
halo_north(s::AbstractCubicalComplex3D) = s.halo_north
halo_down(s::AbstractCubicalComplex3D)  = s.halo_down
halo_up(s::AbstractCubicalComplex3D)    = s.halo_up

nxr(s::AbstractCubicalComplex3D) = s.nx
nyr(s::AbstractCubicalComplex3D) = s.ny
nzr(s::AbstractCubicalComplex3D) = s.nz

nx(s::AbstractCubicalComplex3D) = nxr(s) + halo_west(s) + halo_east(s)
ny(s::AbstractCubicalComplex3D) = nyr(s) + halo_south(s) + halo_north(s)
nz(s::AbstractCubicalComplex3D) = nzr(s) + halo_down(s) + halo_up(s)
...
nxe_r(s::AbstractCubicalComplex3D) = nxr(s) - 1
nye_r(s::AbstractCubicalComplex3D) = nyr(s) - 1
nze_r(s::AbstractCubicalComplex3D) = nzr(s) - 1
...
function valid_xedge(s::AbstractCubicalComplex3D, x, y, z)
  return (1 <= x <= nxe(s)) && (1 <= y <= ny(s)) && (1 <= z <= nz(s))
end
function valid_yedge(s::AbstractCubicalComplex3D, x, y, z)
  return (1 <= x <= nx(s)) && (1 <= y <= nye(s)) && (1 <= z <= nz(s))
end
function valid_zedge(s::AbstractCubicalComplex3D, x, y, z)
  return (1 <= x <= nx(s)) && (1 <= y <= ny(s)) && (1 <= z <= nze(s))
end

function valid_xyquad(s::AbstractCubicalComplex3D, x, y, z)
  return (1 <= x <= nxq(s)) && (1 <= y <= nyq(s)) && (1 <= z <= nz(s))
end
function valid_xzquad(s::AbstractCubicalComplex3D, x, y, z)
  return (1 <= x <= nxq(s)) && (1 <= y <= ny(s)) && (1 <= z <= nzq(s))
end
function valid_yzquad(s::AbstractCubicalComplex3D, x, y, z)
  return (1 <= x <= nx(s)) && (1 <= y <= nyq(s)) && (1 <= z <= nzq(s))
end

function valid_boid(s::AbstractCubicalComplex3D, x, y, z)
  return (1 <= x <= nxb(s)) && (1 <= y <= nyb(s)) && (1 <= z <= nzb(s))
end
...
function coord_to_vert(s::AbstractCubicalComplex3D, x::Int, y::Int, z::Int)
  return x + (y - 1) * nx(s) + (z - 1) * nx(s) * ny(s)
end
...
function coord_to_boid(s::AbstractCubicalComplex3D, x::Int, y::Int, z::Int)
  return x + (y - 1) * nxb(s) + (z - 1) * nxyb(s)
end
...
function vert_to_coord(s::AbstractCubicalComplex3D, idx::Int)
  idx0 = idx - 1
  nxy = nx(s) * ny(s)
...
function edge_to_coord(s::AbstractCubicalComplex3D, idx::Int)
  if idx <= nxedges(s)
    align = X_ALIGN
    idx0 = idx - 1
    nxe_val = nxe(s)
...
function quad_to_coord(s::AbstractCubicalComplex3D, idx::Int)
  if idx <= nxyquads(s)
    align = Z_ALIGN
    idx0 = idx - 1
    nx_q = nxb(s)
...
function boid_to_coord(s::AbstractCubicalComplex3D, idx::Int)
  idx0 = idx - 1
  nxy_b = nxb(s) * nyb(s)
...
function point(s::UniformCubicalComplex3D, x::Int, y::Int, z::Int)
  ...
  pz = base_z(s) + (z - 1 - halo_down(s)) * dz(s)
  return Point3(px, py, pz)
end
point(s::UniformCubicalComplex3D, v::Int) = point(s, vert_to_coord(s, v)...)

function real_point(s::UniformCubicalComplex3D, x::Int, y::Int, z::Int)
...
tgt(s::AbstractCubicalComplex3D, e::Int) = tgt(s, edge_to_coord(s, e)...)
...
edge_len(s::UniformCubicalComplex3D, e::Int) = edge_len(s, edge_to_coord(s, e)...)

xedges(s::AbstractCubicalComplex3D, arr::AbstractVector) = @view arr[1:nxedges(s)]
function yedges(s::AbstractCubicalComplex3D, arr::AbstractVector)
  @view arr[(nxedges(s) + 1):(nxedges(s) + nyedges(s))]
end
function zedges(s::AbstractCubicalComplex3D, arr::AbstractVector)
  @view arr[(nxedges(s) + nyedges(s) + 1):end]
end

xyquads(s::AbstractCubicalComplex3D, arr::AbstractVector) = @view arr[1:nxyquads(s)]
function xzquads(s::AbstractCubicalComplex3D, arr::AbstractVector)
  @view arr[(nxyquads(s) + 1):(nxyquads(s) + nxzquads(s))]
end
function yzquads(s::AbstractCubicalComplex3D, arr::AbstractVector)
  @view arr[(nxyquads(s) + nxzquads(s) + 1):end]
end

# Z-aligned (XY) quads — down and up faces
function down_quads(s::AbstractCubicalComplex3D)
  return [coord_to_quad(s, x, y, 1, Z_ALIGN)
      for x in 1:nxq(s), y in 1:nyq(s)][:]
end

function up_quads(s::AbstractCubicalComplex3D)
  return [coord_to_quad(s, x, y, nz(s), Z_ALIGN)
      for x in 1:nxq(s), y in 1:nyq(s)][:]
end

# Y-aligned (XZ) quads — south and north faces
function south_quads(s::AbstractCubicalComplex3D)
  return [coord_to_quad(s, x, 1, z, Y_ALIGN)
      for x in 1:nxq(s), z in 1:nzq(s)][:]
end

function north_quads(s::AbstractCubicalComplex3D)
  return [coord_to_quad(s, x, ny(s), z, Y_ALIGN)
      for x in 1:nxq(s), z in 1:nzq(s)][:]
end

# X-aligned (YZ) quads — west and east faces
function west_quads(s::AbstractCubicalComplex3D)
  return [coord_to_quad(s, 1, y, z, X_ALIGN)
      for y in 1:nyb(s), z in 1:nzb(s)][:]
end

function east_quads(s::AbstractCubicalComplex3D)
  return [coord_to_quad(s, nx(s), y, z, X_ALIGN)
      for y in 1:nyb(s), z in 1:nzb(s)][:]
end

function boundary_quads(s::AbstractCubicalComplex3D)
...
function dual_point(s::UniformCubicalComplex3D{FT}, x::Int, y::Int, z::Int) where FT <: AbstractFloat
  px = base_x(s) + (x - FT(0.5) - halo_west(s)) * dx(s)
  py = base_y(s) + (y - FT(0.5) - halo_south(s)) * dy(s)
  pz = base_z(s) + (z - FT(0.5) - halo_down(s)) * dz(s)
  return Point3(px, py, pz)
end
dual_points(s::UniformCubicalComplex3D) = map(v -> dual_point(s, boid_to_coord(s, v)...), boids(s))

function real_dual_point(s::UniformCubicalComplex3D, rx::Int, ry::Int, rz::Int)
  return dual_point(s, rx + halo_west(s), ry + halo_south(s), rz + halo_down(s))
end

function real_coord_to_boid(s::AbstractCubicalComplex3D, rx::Int, ry::Int, rz::Int)
  return coord_to_boid(s, rx + halo_west(s), ry + halo_south(s), rz + halo_down(s))
end

function dual_edge_len(s::UniformCubicalComplex3D{FT}, x::Int, y::Int, z::Int, align::Align) where FT <: AbstractFloat
  if align == Z_ALIGN
...
dual_edge_len(s::AbstractCubicalComplex3D, e::Int) = dual_edge_len(s, edge_to_coord(s, e)...)

function dual_quad_area(s::UniformCubicalComplex3D{FT}, x::Int, y::Int, z::Int, align::Align) where FT <: AbstractFloat
  lx = (x == 1 || x == nx(s)) ? dx(s) / FT(2.0) : dx(s)
  ly = (y == 1 || y == ny(s)) ? dy(s) / FT(2.0) : dy(s)
  lz = (z == 1 || z == nz(s)) ? dz(s) / FT(2.0) : dz(s)
...
function dual_boid_volume(s::UniformCubicalComplex3D{FT}, x::Int, y::Int, z::Int) where FT <: AbstractFloat
  lx = (x == 1 || x == nx(s)) ? dx(s) / FT(2.0) : dx(s)
  ly = (y == 1 || y == ny(s)) ? dy(s) / FT(2.0) : dy(s)
  lz = (z == 1 || z == nz(s)) ? dz(s) / FT(2.0) : dz(s)
...
function edge_quads(s::UniformCubicalComplex3D, x::Int, y::Int, z::Int, align::Align)
  if align == X_ALIGN
    v1 = valid_xyquad(s, x, y - 1, z)
    q1 = v1 ? coord_to_quad(s, x, y - 1, z, Z_ALIGN) : 0
    v2 = valid_xzquad(s, x, y, z - 1)
    q2 = v2 ? coord_to_quad(s, x, y, z - 1, Y_ALIGN) : 0
    v3 = valid_xyquad(s, x, y, z)
    q3 = v3 ? coord_to_quad(s, x, y, z, Z_ALIGN) : 0
    v4 = valid_xzquad(s, x, y, z)
    q4 = v4 ? coord_to_quad(s, x, y, z, Y_ALIGN) : 0
  elseif align == Y_ALIGN
    v1 = valid_yzquad(s, x, y, z - 1)
    q1 = v1 ? coord_to_quad(s, x, y, z - 1, X_ALIGN) : 0
    v2 = valid_xyquad(s, x - 1, y, z)
    q2 = v2 ? coord_to_quad(s, x - 1, y, z, Z_ALIGN) : 0
    v3 = valid_yzquad(s, x, y, z)
    q3 = v3 ? coord_to_quad(s, x, y, z, X_ALIGN) : 0
    v4 = valid_xyquad(s, x, y, z)
    q4 = v4 ? coord_to_quad(s, x, y, z, Z_ALIGN) : 0
  else # align == Z_ALIGN
    v1 = valid_xzquad(s, x - 1, y, z)
    q1 = v1 ? coord_to_quad(s, x - 1, y, z, Y_ALIGN) : 0
    v2 = valid_yzquad(s, x, y - 1, z)
    q2 = v2 ? coord_to_quad(s, x, y - 1, z, X_ALIGN) : 0
    v3 = valid_xzquad(s, x, y, z)
    q3 = v3 ? coord_to_quad(s, x, y, z, Y_ALIGN) : 0
    v4 = valid_yzquad(s, x, y, z)
    q4 = v4 ? coord_to_quad(s, x, y, z, X_ALIGN) : 0
  end
...
function edge_boids(s::UniformCubicalComplex3D, x::Int, y::Int, z::Int, align::Align)
  if align == X_ALIGN
    ...
    v1 = valid_boid(s, x, y - 1, z - 1)
    b1_idx = v1 ? coord_to_boid(s, x, y - 1, z - 1) : 0
    v2 = valid_boid(s, x, y, z - 1)
    b2_idx = v2 ? coord_to_boid(s, x, y, z - 1) : 0
    v3 = valid_boid(s, x, y, z)
    b3_idx = v3 ? coord_to_boid(s, x, y, z) : 0
    v4 = valid_boid(s, x, y - 1, z)
    b4_idx = v4 ? coord_to_boid(s, x, y - 1, z) : 0
  elseif align == Y_ALIGN
    # Boids around a Y-aligned edge (x,z plane)
    v1 = valid_boid(s, x, y, z - 1)
    b1_idx = v1 ? coord_to_boid(s, x, y, z - 1) : 0
    v2 = valid_boid(s, x - 1, y, z - 1)
    b2_idx = v2 ? coord_to_boid(s, x - 1, y, z - 1) : 0
    v3 = valid_boid(s, x - 1, y, z)
    b3_idx = v3 ? coord_to_boid(s, x - 1, y, z) : 0
    v4 = valid_boid(s, x, y, z)
    b4_idx = v4 ? coord_to_boid(s, x, y, z) : 0
  else # align == Z_ALIGN
    # Boids around a Z-aligned edge (x,y plane)
    v1 = valid_boid(s, x - 1, y - 1, z)
    b1_idx = v1 ? coord_to_boid(s, x - 1, y - 1, z) : 0
    v2 = valid_boid(s, x, y - 1, z)
    b2_idx = v2 ? coord_to_boid(s, x, y - 1, z) : 0
    v3 = valid_boid(s, x, y, z)
    b3_idx = v3 ? coord_to_boid(s, x, y, z) : 0
    v4 = valid_boid(s, x - 1, y, z)
    b4_idx = v4 ? coord_to_boid(s, x - 1, y, z) : 0
  end
...
function vertex_edges(s::UniformCubicalComplex3D, x::Int, y::Int, z::Int)
  v_low = valid_zedge(s, x, y, z - 1)
  e_low = v_low ? coord_to_edge(s, x, y, z - 1, Z_ALIGN) : 0
  v_high = valid_zedge(s, x, y, z)
  e_high = v_high ? coord_to_edge(s, x, y, z, Z_ALIGN) : 0
  v_south = valid_yedge(s, x, y - 1, z)
  e_south = v_south ? coord_to_edge(s, x, y - 1, z, Y_ALIGN) : 0
  v_north = valid_yedge(s, x, y, z)
  e_north = v_north ? coord_to_edge(s, x, y, z, Y_ALIGN) : 0
  v_west = valid_xedge(s, x - 1, y, z)
  e_west = v_west ? coord_to_edge(s, x - 1, y, z, X_ALIGN) : 0
  v_east = valid_xedge(s, x, y, z)
  e_east = v_east ? coord_to_edge(s, x, y, z, X_ALIGN) : 0
...
function ghost_boids(s::AbstractCubicalComplex3D)
  function low_slab(n_ax, n_b, n_c, h_low, to_idx)
    h_low == 0 && return (send = Int[], recv = Int[])
    return (
      send = [to_idx(ax, b, c) for ax in (h_low + 1):(2h_low), b in 1:n_b, c in 1:n_c][:],
      recv = [to_idx(ax, b, c) for ax in 1:h_low,              b in 1:n_b, c in 1:n_c][:],
    )
  end

  function high_slab(n_ax, n_b, n_c, h_high, to_idx)
    h_high == 0 && return (send = Int[], recv = Int[])
    return (
      send = [to_idx(ax, b, c) for ax in (n_ax - 2h_high + 1):(n_ax - h_high), b in 1:n_b, c in 1:n_c][:],
      recv = [to_idx(ax, b, c) for ax in (n_ax - h_high + 1):n_ax,             b in 1:n_b, c in 1:n_c][:],
    )
  end

  ew = (ax, b, c) -> coord_to_boid(s, ax, b, c)
  ns = (ax, b, c) -> coord_to_boid(s, b, ax, c)
  ud = (ax, b, c) -> coord_to_boid(s, b, c, ax)

  return (
    west  = low_slab( nxb(s), nyb(s), nzb(s), halo_west(s),  ew),
    east  = high_slab(nxb(s), nyb(s), nzb(s), halo_east(s),  ew),
    south = low_slab( nyb(s), nxb(s), nzb(s), halo_south(s), ns),
    north = high_slab(nyb(s), nxb(s), nzb(s), halo_north(s), ns),
    down  = low_slab( nzb(s), nxb(s), nyb(s), halo_down(s),  ud),
    up    = high_slab(nzb(s), nxb(s), nyb(s), halo_up(s),    ud),
  )
end
...
function interior(::Val{2}, f::AbstractVector, s::AbstractCubicalComplex3D)
  ...
  xy_indices = [
    ...
    for z in (halo_down(s) + 1):(halo_down(s) + nzr(s))
    for y in (halo_south(s) + 1):(halo_south(s) + nybr(s))
    for x in (halo_west(s) + 1):(halo_west(s) + nxbr(s))
  ]
  xz_indices = [
    coord_to_quad(s, x, y, z, Y_ALIGN)
    for z in (halo_down(s) + 1):(halo_down(s) + nzbr(s))
    for y in (halo_south(s) + 1):(halo_south(s) + nyr(s))
    for x in (halo_west(s) + 1):(halo_west(s) + nxbr(s))
  ]
  yz_indices = [
    ...
    for z in (halo_down(s) + 1):(halo_down(s) + nzbr(s))
    for y in (halo_south(s) + 1):(halo_south(s) + nybr(s))
    for x in (halo_west(s) + 1):(halo_west(s) + nxr(s))
  ]
