...
# Compact scalar stencil container used by the scalar fallback API.
struct RectStencil{FT <: AbstractFloat, N} <: AbstractStencil
  f::NTuple{N, FT}
end

RectStencil{FT}(args::Vararg{FT, N}) where {FT <: AbstractFloat, N} = RectStencil{FT, N}(args)
RectStencil{FT}(vec::AbstractVector) where {FT <: AbstractFloat} = RectStencil{FT, length(vec)}(Tuple(FT.(vec)))

function substencil(weno::UniformWENO, s::RectStencil{FT}, j::Int) where {FT <: AbstractFloat}
  r = polyorder(weno)
  return ntuple(i -> s.f[j + i - 1], r + 1)
end
...
ISW(::UniformWENO{5, FT}, ::Val{1}) where {FT} = FT.(ISW5_1)
ISW(::UniformWENO{5, FT}, ::Val{2}) where {FT} = FT.(ISW5_2)
ISW(::UniformWENO{5, FT}, ::Val{3}) where {FT} = FT.(ISW5_3)
CW(::UniformWENO{5, FT}, ::Val{1}) where {FT} = FT.(CW5_1)
CW(::UniformWENO{5, FT}, ::Val{2}) where {FT} = FT.(CW5_2)
CW(::UniformWENO{5, FT}, ::Val{3}) where {FT} = FT.(CW5_3)
d_r(::UniformWENO{5, FT}) where {FT} = FT.(DR5)

ISW(::UniformWENO{7, FT}, ::Val{1}) where {FT} = FT.(ISW7_1)
ISW(::UniformWENO{7, FT}, ::Val{2}) where {FT} = FT.(ISW7_2)
ISW(::UniformWENO{7, FT}, ::Val{3}) where {FT} = FT.(ISW7_3)
ISW(::UniformWENO{7, FT}, ::Val{4}) where {FT} = FT.(ISW7_4)
CW(::UniformWENO{7, FT}, ::Val{1}) where {FT} = FT.(CW7_1)
CW(::UniformWENO{7, FT}, ::Val{2}) where {FT} = FT.(CW7_2)
CW(::UniformWENO{7, FT}, ::Val{3}) where {FT} = FT.(CW7_3)
CW(::UniformWENO{7, FT}, ::Val{4}) where {FT} = FT.(CW7_4)
d_r(::UniformWENO{7, FT}) where {FT} = FT.(DR7)

ISW(::UniformWENO{9, FT}, ::Val{1}) where {FT} = FT.(ISW9_1)
ISW(::UniformWENO{9, FT}, ::Val{2}) where {FT} = FT.(ISW9_2)
ISW(::UniformWENO{9, FT}, ::Val{3}) where {FT} = FT.(ISW9_3)
ISW(::UniformWENO{9, FT}, ::Val{4}) where {FT} = FT.(ISW9_4)
ISW(::UniformWENO{9, FT}, ::Val{5}) where {FT} = FT.(ISW9_5)
CW(::UniformWENO{9, FT}, ::Val{1}) where {FT} = FT.(CW9_1)
CW(::UniformWENO{9, FT}, ::Val{2}) where {FT} = FT.(CW9_2)
CW(::UniformWENO{9, FT}, ::Val{3}) where {FT} = FT.(CW9_3)
CW(::UniformWENO{9, FT}, ::Val{4}) where {FT} = FT.(CW9_4)
CW(::UniformWENO{9, FT}, ::Val{5}) where {FT} = FT.(CW9_5)
d_r(::UniformWENO{9, FT}) where {FT} = FT.(DR9)

# -----------------------------------------------------------------------------
# Core arithmetic helpers
# -----------------------------------------------------------------------------

@inline function beta3(s1::FT, s2::FT, s3::FT, c::NTuple{6, <:Real}) where {FT}
  return FT(c[1]) * s1 * s1 + FT(c[2]) * s1 * s2 + FT(c[3]) * s1 * s3 +
         FT(c[4]) * s2 * s2 + FT(c[5]) * s2 * s3 +
         FT(c[6]) * s3 * s3
end

@inline function beta4(s1::FT, s2::FT, s3::FT, s4::FT, c::NTuple{10, <:Real}) where {FT}
  return FT(c[1]) * s1 * s1 + FT(c[2]) * s1 * s2 + FT(c[3]) * s1 * s3 + FT(c[4]) * s1 * s4 +
         FT(c[5]) * s2 * s2 + FT(c[6]) * s2 * s3 + FT(c[7]) * s2 * s4 +
         FT(c[8]) * s3 * s3 + FT(c[9]) * s3 * s4 +
         FT(c[10]) * s4 * s4
end

@inline function beta5(s1::FT, s2::FT, s3::FT, s4::FT, s5::FT, c::NTuple{15, <:Real}) where {FT}
  return FT(c[1]) * s1 * s1 + FT(c[2]) * s1 * s2 + FT(c[3]) * s1 * s3 + FT(c[4]) * s1 * s4 + FT(c[5]) * s1 * s5 +
         FT(c[6]) * s2 * s2 + FT(c[7]) * s2 * s3 + FT(c[8]) * s2 * s4 + FT(c[9]) * s2 * s5 +
         FT(c[10]) * s3 * s3 + FT(c[11]) * s3 * s4 + FT(c[12]) * s3 * s5 +
         FT(c[13]) * s4 * s4 + FT(c[14]) * s4 * s5 +
         FT(c[15]) * s5 * s5
end

@inline function weno5_point(fm2::FT, fm1::FT, f0::FT, fp1::FT, fp2::FT, eps::FT) where {FT <: AbstractFloat}
  q1 = FT(CW5_1[1]) * fm2 + FT(CW5_1[2]) * fm1 + FT(CW5_1[3]) * f0
  q2 = FT(CW5_2[1]) * fm1 + FT(CW5_2[2]) * f0 + FT(CW5_2[3]) * fp1
  q3 = FT(CW5_3[1]) * f0 + FT(CW5_3[2]) * fp1 + FT(CW5_3[3]) * fp2

  b1 = beta3(fm2, fm1, f0, ISW5_1)
  b2 = beta3(fm1, f0, fp1, ISW5_2)
  b3 = beta3(f0, fp1, fp2, ISW5_3)

  a1 = FT(DR5[1]) / ((eps + b1) * (eps + b1))
  a2 = FT(DR5[2]) / ((eps + b2) * (eps + b2))
  a3 = FT(DR5[3]) / ((eps + b3) * (eps + b3))
...
@inline function weno7_point(fm3::FT, fm2::FT, fm1::FT, f0::FT, fp1::FT, fp2::FT, fp3::FT, eps::FT) where {FT <: AbstractFloat}
  q1 = FT(CW7_1[1]) * fm3 + FT(CW7_1[2]) * fm2 + FT(CW7_1[3]) * fm1 + FT(CW7_1[4]) * f0
  q2 = FT(CW7_2[1]) * fm2 + FT(CW7_2[2]) * fm1 + FT(CW7_2[3]) * f0 + FT(CW7_2[4]) * fp1
  q3 = FT(CW7_3[1]) * fm1 + FT(CW7_3[2]) * f0 + FT(CW7_3[3]) * fp1 + FT(CW7_3[4]) * fp2
  q4 = FT(CW7_4[1]) * f0 + FT(CW7_4[2]) * fp1 + FT(CW7_4[3]) * fp2 + FT(CW7_4[4]) * fp3

  b1 = beta4(fm3, fm2, fm1, f0, ISW7_1)
  b2 = beta4(fm2, fm1, f0, fp1, ISW7_2)
  b3 = beta4(fm1, f0, fp1, fp2, ISW7_3)
  b4 = beta4(f0, fp1, fp2, fp3, ISW7_4)

  a1 = FT(DR7[1]) / ((eps + b1) * (eps + b1))
  a2 = FT(DR7[2]) / ((eps + b2) * (eps + b2))
  a3 = FT(DR7[3]) / ((eps + b3) * (eps + b3))
  a4 = FT(DR7[4]) / ((eps + b4) * (eps + b4))
...
@inline function weno9_point(fm4::FT, fm3::FT, fm2::FT, fm1::FT, f0::FT, fp1::FT, fp2::FT, fp3::FT, fp4::FT, eps::FT) where {FT <: AbstractFloat}
  q1 = FT(CW9_1[1]) * fm4 + FT(CW9_1[2]) * fm3 + FT(CW9_1[3]) * fm2 + FT(CW9_1[4]) * fm1 + FT(CW9_1[5]) * f0
  q2 = FT(CW9_2[1]) * fm3 + FT(CW9_2[2]) * fm2 + FT(CW9_2[3]) * fm1 + FT(CW9_2[4]) * f0 + FT(CW9_2[5]) * fp1
  q3 = FT(CW9_3[1]) * fm2 + FT(CW9_3[2]) * fm1 + FT(CW9_3[3]) * f0 + FT(CW9_3[4]) * fp1 + FT(CW9_3[5]) * fp2
  q4 = FT(CW9_4[1]) * fm1 + FT(CW9_4[2]) * f0 + FT(CW9_4[3]) * fp1 + FT(CW9_4[4]) * fp2 + FT(CW9_4[5]) * fp3
  q5 = FT(CW9_5[1]) * f0 + FT(CW9_5[2]) * fp1 + FT(CW9_5[3]) * fp2 + FT(CW9_5[4]) * fp3 + FT(CW9_5[5]) * fp4

  b1 = beta5(fm4, fm3, fm2, fm1, f0, ISW9_1)
  b2 = beta5(fm3, fm2, fm1, f0, fp1, ISW9_2)
  b3 = beta5(fm2, fm1, f0, fp1, fp2, ISW9_3)
  b4 = beta5(fm1, f0, fp1, fp2, fp3, ISW9_4)
  b5 = beta5(f0, fp1, fp2, fp3, fp4, ISW9_5)

  a1 = FT(DR9[1]) / ((eps + b1) * (eps + b1))
  a2 = FT(DR9[2]) / ((eps + b2) * (eps + b2))
  a3 = FT(DR9[3]) / ((eps + b3) * (eps + b3))
  a4 = FT(DR9[4]) / ((eps + b4) * (eps + b4))
  a5 = FT(DR9[5]) / ((eps + b5) * (eps + b5))
...
function flux(weno::UniformWENO, stencil::RectStencil{FT}, i::Int) where {FT <: AbstractFloat}
  vals = substencil(weno, stencil, i)
  cs = CW(weno, Val(i))
...
function WENO(weno::UniformWENO{5, FT}, stencil::RectStencil{FT}; eps::FT = FT(1e-6)) where {FT <: AbstractFloat}
  f = stencil.f
  return weno5_point(f[1], f[2], f[3], f[4], f[5], eps)
end

function WENO(weno::UniformWENO{7, FT}, stencil::RectStencil{FT}; eps::FT = FT(1e-6)) where {FT <: AbstractFloat}
  f = stencil.f
  return weno7_point(f[1], f[2], f[3], f[4], f[5], f[6], f[7], eps)
end

function WENO(weno::UniformWENO{9, FT}, stencil::RectStencil{FT}; eps::FT = FT(1e-6)) where {FT <: AbstractFloat}
  f = stencil.f
  return weno9_point(f[1], f[2], f[3], f[4], f[5], f[6], f[7], f[8], f[9], eps)
end
