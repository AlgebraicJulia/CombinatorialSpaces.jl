...
struct UniformDECCache3D{
  ...
  #   q3, q4 = South, North
  #   q5, q6 = West, East
  # TODO: w12_q1..w12_q6 duplicate b_q1..b_q6 exactly (both are
  # boid_quads(s, boid_to_coord(s, b)...) over 1:nboids_). Drop these six
  # fields and have kernel_wedge_12_cached_3d! take cache.b_q1..b_q6 instead.
  w12_q1  :: IT
  w12_q2  :: IT
  w12_q3  :: IT
  w12_q4  :: IT
...
@kernel function kernel_wedge_11_cached_3d!(
  ...
  @inbounds begin
    FT = eltype(a)
    a1 = (a[q_e1[q]] + a[q_e3[q]]) * FT(0.5)
    a2 = (a[q_e2[q]] + a[q_e4[q]]) * FT(0.5)
    b1 = (b[q_e1[q]] + b[q_e3[q]]) * FT(0.5)
    b2 = (b[q_e2[q]] + b[q_e4[q]]) * FT(0.5)
    res[q] = a1 * b2 - a2 * b1
  end
end
...
@kernel function kernel_wedge_12_cached_3d!(
  ...
  @inbounds begin
    FT = eltype(a)
    ax = FT(0.25) * (a[w12_e1[idx]] + a[w12_e2[idx]] + a[w12_e3[idx]] + a[w12_e4[idx]])
    ay = FT(0.25) * (a[w12_e5[idx]] + a[w12_e6[idx]] + a[w12_e7[idx]] + a[w12_e8[idx]])
    az = FT(0.25) * (a[w12_e9[idx]] + a[w12_e10[idx]] + a[w12_e11[idx]] + a[w12_e12[idx]])

    bz = FT(0.5) * (b[w12_q1[idx]] + b[w12_q2[idx]])
...
@kernel function kernel_wedge_dd_01_cached_3d!(
  ...
  @inbounds begin
    ...
    avg = ifelse(
      nvalid == Int8(2), (pos + neg) * eltype(f)(0.5),
