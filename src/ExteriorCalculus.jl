...
"""
@theory ThAdditiveMonoidalCategory <: ThAdditiveCategory begin
  ...
  @op (⊗) := otimes
  otimes(A::Ob, B::Ob)::Ob
  otimes(f::(A → B), g::(C → D))::((A ⊗ C) → (B ⊗ D)) ⊣ [(A,B,C,D)::Ob]
  munit()::Ob
  ...
  (A ⊕ B) ⊗ C == (A ⊗ C) ⊕ (B ⊗ C) ⊣ [(A,B,C)::Ob]
  mzero() ⊗ A == mzero() ⊣ [A::Ob]
  A ⊗ (B ⊕ C) == (A ⊗ B) ⊕ (A ⊗ C) ⊣ [(A,B,C)::Ob]
  A ⊗ mzero() == mzero() ⊣ [A::Ob]

  # Bi-additivity.
  @op (+) := plus
  (f + g) ⊗ h == (f ⊗ h) + (g ⊗ h) ⊣
    [(A,B,C,D)::Ob, f::(A → B), g::(A → B), h::(C → D)]
  (delete(A) ⋅ zero(B)) ⊗ f == delete(A⊗C) ⋅ zero(B⊗D) ⊣
    [(A,B,C,D)::Ob, f::(C → D)]
  f ⊗ (g + h) == (f ⊗ g) + (f ⊗ h) ⊣
    [(A,B,C,D)::Ob, f::(A → B), g::(C → D), h::(C → D)]
...
"""
@theory ThManifoldCalculus <: ThAdditiveMonoidalCategory begin
  Space::TYPE
...
"""
@theory ThMetricFreeExtCalc1D₊ <: ThManifoldCalculus begin
  Chain0(X::Space)::Ob
  Chain1(X::Space)::Ob
  ...
  Form0(X::Space)::Ob
  Form1(X::Space)::Ob
  Form0(X⊔Y) == Form0(X)⊕Form0(Y) ⊣ [(X,Y)::Space]
  Form1(X⊔Y) == Form1(X)⊕Form1(Y) ⊣ [(X,Y)::Space]
...
"""
@theory ThMetricFreeExtCalc1D <: ThMetricFreeExtCalc1D₊ begin
  ℒ₁(X) == ι₁(X) ⋅ d₀(X) ⊣ [X::Space]
end

""" Theory of exterior calculus on 2-or-higher-dimensional manifold-like spaces.
"""
@theory ThMetricFreeExtCalc2D₊ <: ThMetricFreeExtCalc1D₊ begin
  Chain2(X::Space)::Ob
  ∂₂(X::Space)::Hom(Chain2(X), Chain1(X))
  ∂₂(X) ⋅ ∂₁(X) == zero(Chain2(X), Chain0(X)) ⊣ [X::Space]

  Form2(X::Space)::Ob
  Form2(X⊔Y) == Form2(X)⊕Form2(Y) ⊣ [(X,Y)::Space]
...
"""
@theory ThMetricFreeExtCalc2D <: ThMetricFreeExtCalc2D₊ begin
  ℒ₂(X) == ι₂(X) ⋅ d₁(X) ⊣ [X::Space]
end
...
"""
@theory ThExtCalc1D <: ThMetricFreeExtCalc1D begin
  DualForm0(X::Space)::Ob
  DualForm1(X::Space)::Ob
  DualForm0(X⊔Y) == DualForm0(X)⊕DualForm0(Y) ⊣ [(X,Y)::Space]
  DualForm1(X⊔Y) == DualForm1(X)⊕DualForm1(Y) ⊣ [(X,Y)::Space]
  ...
  ⋆₀(X) ⋅ ⋆₀⁻¹(X) == id(Form0(X)) ⊣ [X::Space]
  ⋆₁(X) ⋅ ⋆₁⁻¹(X) == id(Form1(X)) ⊣ [X::Space]

  δ₁(X::Space)::Hom(Form1(X), Form0(X))
  δ₁(X) == ⋆₁(X) ⋅ dual_d₀(X) ⋅ ⋆₀⁻¹(X) ⋅ antipode(Form0(X)) ⊣ [X::Space]
...
@symbolic_model FreeExtCalc1D{ObExpr,HomExpr,GATExpr} ThExtCalc1D begin
...
"""
@theory ThExtCalc2D <: ThMetricFreeExtCalc2D begin
  DualForm0(X::Space)::Ob
  DualForm1(X::Space)::Ob
  DualForm2(X::Space)::Ob
  DualForm0(X⊔Y) == DualForm0(X)⊕DualForm0(Y) ⊣ [(X,Y)::Space]
  DualForm1(X⊔Y) == DualForm1(X)⊕DualForm1(Y) ⊣ [(X,Y)::Space]
  DualForm2(X⊔Y) == DualForm2(X)⊕DualForm2(Y) ⊣ [(X,Y)::Space]
  dual_d₀(X::Space)::Hom(DualForm0(X), DualForm1(X))
  ...
  ⋆₀(X) ⋅ ⋆₀⁻¹(X) == id(Form0(X)) ⊣ [X::Space]
  ⋆₁(X) ⋅ ⋆₁⁻¹(X) == antipode(Form1(X)) ⊣ [X::Space]
  ⋆₂(X) ⋅ ⋆₂⁻¹(X) == id(Form1(X)) ⊣ [X::Space]

  δ₁(X::Space)::Hom(Form1(X), Form0(X))
  δ₂(X::Space)::Hom(Form2(X), Form1(X))
  δ₁(X) == ⋆₁(X) ⋅ dual_d₁(X) ⋅ ⋆₀⁻¹(X) ⋅ antipode(Form0(X)) ⊣ [X::Space]
  δ₂(X) == ⋆₂(X) ⋅ dual_d₀(X) ⋅ ⋆₁⁻¹(X) ⊣ [X::Space]
...
@symbolic_model FreeExtCalc2D{ObExpr,HomExpr,GATExpr} ThExtCalc2D begin
  compose(f::Hom, g::Hom) = associate_unit(new(f,g; strict=true), id)
  oplus(A::Ob, B::Ob) = associate_unit(new(A,B), mzero)
  oplus(f::Hom, g::Hom) = associate(new(f,g))
  otimes(A::Ob, B::Ob) = associate_unit(new(A,B), munit)
