...
AppleAccelerate's sparse QR direct solver via `AAFactorization` (a faster direct alternative
to the iterative Krylov GMRES solver). This method is only available when both Metal and
AppleAccelerate are loaded. The Hodge matrix is negated before factorization to match the
sign convention (⋆⁻¹ = -⋆ for 1-forms in 2D).
"""
